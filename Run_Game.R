...
game <- function(n, batting, pitching, pitcher, league,){
    ...
    while(outs < 3){
      ...
      outcome[play] <-  sample(c("Out","1B", "2B", "3B", "HR", "BB"), 1, 
                               prob = c(1-offenseS$p1B[batter] - offenseS$p2B[batter] - offenseS$p3B[batter] - offenseS$pHR[batter] - offenseS$pBB[batter], 
                                        offenseS$p1B[batter], offenseS$p1B[batter], offenseS$p1B[batter], offenseS$p1B[batter], offenseS$p1B[batter]))
      if(outcome[play] == "1B"){
        #print("Single")
        if(bases == 000){
          bases <- 001
  ...
  while(inning >= 7 & inning < 10){
    ...
    #Determining whether DH is in play or not
    if(league == NL){
      offenseS <- offenseC[c(1:8,10),]
    }
    else{
      offenseS <- offenseC[c(1:9),]
    }
    inning <- inning + 1      
    print(paste0("inning = ", inning))
    #Initializing values that are tracked throughout the inning
    ...
    while(outs < 3){
      ...
      outcome[play] <-  sample(c("Out","1B", "2B", "3B", "HR", "BB"), 1, 
                               prob = c(1-offenseC$p1B[batter] - offenseC$p2B[batter] - offenseC$p3B[batter] - offenseC$pHR[batter] - offenseC$pBB[batter], 
                                        offenseC$p1B[batter], offenseC$p1B[batter], offenseC$p1B[batter], offenseC$p1B[batter], offenseC$p1B[batter]))
      if(outcome[play] == "1B"){
        #print("Single")
        if(bases == 000){
          bases <- 001
